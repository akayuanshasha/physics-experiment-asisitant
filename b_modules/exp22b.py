...
def handle(workpath,extension):
    ...
    try:
        ...
        if extension=='csv':
            ...
            data=pd.read_csv(excelpath, header=0, names=["L","d","k","x"], encoding=encode) # 读取csv文件
        else:
            data=pd.read_excel(excelpath, header=0, names=["L","d","k","x"]) # 读取xls/xlsx文件
        ...
        docu.add_paragraph("双缝衍射") # 在Word文档中添加文字
        docu.add_paragraph()
