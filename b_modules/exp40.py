...
def schema():
    ...
    return make_schema(
        (
            "本实验测量金属丝的杨氏模量及泊松比。请记录轴向与横向形变数据（" r"$F$" "、"
            r"$\Delta L$" "、" r"$\Delta D$" "），按 " r"$E = \sigma/\varepsilon$" "、"
            r"$\mu = -\varepsilon_{\text{横}}/\varepsilon_{\text{纵}}$" " 计算，并与合金钢典型值"
            "（" r"$E \approx 190$" r"~$220$" " GPa、" r"$\mu \approx 0.24$" r"~$0.33$" "）对照。"
        ),
        [table], analysis_hints="检查形变与载荷的线性及横向形变符号。",
