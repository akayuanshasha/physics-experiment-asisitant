...
def schema():
    chart = {
        ...
        "x_label": "砝码序号 n", "y_label": "标尺读数 b̄ (cm)",
        "title": "b̄-序号 关系图", "fit": "linear",
    }
    # 表格1：钢丝直径（示例数据取自 exp5 示例 CSV）
    table1 = _set_units(
        make_table(
            "table1", "钢丝直径测量表",
            ["d_i(mm)"],
            sample=[[0.291], [0.292], [0.293], [0.294], [0.295]],
            min_rows=3, initial_rows=5,
            description="输入多次用螺旋测微器测得的钢丝直径。",
        ),
        ["mm"],
    )
    # 表格2：光杠杆拉伸形变数据（示例数据取自 exp5 示例 CSV）
    table2 = _set_units(
        make_table(
            "table2", "光杠杆拉伸形变数据表",
            ["b₊(cm)", "b₋(cm)", "b̄(cm)", "Δb(cm)"],
            sample=[
                [3.81, 3.82, "", ""],
                [2.50, 2.49, "", ""],
                [1.20, 1.20, "", ""],
            ...
            readonly=(2, 3), min_rows=3, initial_rows=8, chart=chart,
            description="输入加、减砝码时标尺读数 b₊、b₋，平均值与位移由后端自动计算。",
        ),
        ["cm", "cm", "cm", "cm"],
    )
    return make_schema(
        (
            "本实验用光杠杆放大法（伸长法）测量金属丝的杨氏模量。请记录逐次加（减）砝码时的"
            "光杠杆读数与钢丝直径，按 " r"$E = \frac{8FLD}{\pi d^2 b\Delta n}$" " 计算杨氏模量，"
            "并用最小二乘/作图法处理数据、评定不确定度。"
        ),
        [table1, table2],
        parameters=[
            {"id": "D", "label": "镜尺距 D", "unit": "cm", "type": "number", "default": 128.0},
            {"id": "l", "label": "光杠杆臂长 l", "unit": "cm", "type": "number", "default": 7.2},
            {"id": "L", "label": "钢丝原长 L", "unit": "cm", "type": "number", "default": 100.5},
        ],
        analysis_hints="检查钢丝直径的重复性、光杠杆数据的线性度，以及杨氏模量的合理性。",
        preview_enabled=True,
        formulas=get_formulas("exp5"),
        variables=get_variables("exp5"),
