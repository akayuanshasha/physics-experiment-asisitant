...
def handle(workpath,extension):
    ...
    try:
        ...
        res_sigma=analyse_com("σ=(2*k*Δl)/(2*pi*d)",(("Δl",res_l.average,res_l.unc),("d",res_d.average,res_d.unc)),(("k",k),),"N/m")
        ...
        insert_data(docu, "铁丝圈直径d", res_d, "word")
        ...
        insert_data(docu, "铁丝圈直径d", res_d, "latex")
...
def schema():
    """本实验唯一的前端输入结构与示例数据来源。"""
    return make_schema(
        "表面张力（用金属圈测量）的数据输入与处理。",
        [make_table("table1", "表面张力（用金属圈测量）数据", ["弹簧劲度系数 N/m","金属圈直径/cm","l_0/cm","极限位置刻度/cm"],
                    sample=[["0.85445","3.32","1.24","2.15"],["","3.31","","2.15"],["","3.39","","2.08"],["","","","2.11"],["","","","2.13"]])],
