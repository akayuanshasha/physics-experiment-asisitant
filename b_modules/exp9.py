...
def schema():
    ...
    return {
        ...
        "parameters": [],
        "tables": [],
        "formulas": get_formulas("exp9"),
        "variables": get_variables("exp9"),
...
def handle(workpath,extension):
    ...
    try:
        ...
        if extension=='csv':
            ...
            data=pd.read_csv(excelpath, header=0, names=["L","f","t"], encoding=encode) # 读取csv文件
        else:
            data=pd.read_excel(excelpath, header=0, names=["L","f","t"]) # 读取xls/xlsx文件
        ...
        v_t=analyse_com("v_t=v_0*sqrt(1+t/t_0)", (), (("t", data["t"][0]),("v_0", 331.45), ("t_0", 273.15)), "m/s")
        docu.add_paragraph("实验温度下声速的理论值v_t")
        docu.add_paragraph()._element.append(latex_to_word(v_t.ansx2))
        delta=analyse_com("δ=Abs(v-v_t)/v_t", (), (("v", v.ans), ("v_t", v_t.ans)), "")
        docu.add_paragraph("相对误差δ="+'%.5g'%(delta.ans*100)+"%")
        docu.add_paragraph()._element.append(latex_to_word(delta.ansx2))
        docu.add_paragraph()
        ...
        docu.add_paragraph()
        docu.add_paragraph("实验温度下声速的理论值v_t")
        docu.add_paragraph(v_t.ansx)
        docu.add_paragraph("相对误差δ="+'%.5g'%(delta.ans*100)+"%")
        docu.add_paragraph(delta.ansx)
        docu.add_paragraph()
