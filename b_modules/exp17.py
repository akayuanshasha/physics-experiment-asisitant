...
def schema():
    ...
    return {
        ...
        "description": "数字体温计（PN结温度传感器）",
        "parameters": [],
        "tables": [],
        "formulas": get_formulas("exp17"),
        "variables": get_variables("exp17"),
        "table_theory": get_table_theory("exp17"),
    }
...
def handle(workpath,extension):
    ...
    try:
        ...
        ax.plot(data["t"], data["u"], "o", color='r', markersize=3) # 绘制数据点
        ax.plot(data["t"], res_lsm.b + res_lsm.m*data["t"], color='b', linewidth=1.5) # 拟合直线
        # 作图，详见 https://www.runoob.com/matplotlib/matplotlib-marker.html 和 https://www.runoob.com/matplotlib/matplotlib-line.html
        ax.set_title("电压与温度 U-T 关系曲线", fontproperties=zhfont) # 若有中文，需加fontproperties=zhfont
        ...
        docu.add_paragraph("数字温度计") # 在Word文档中添加文字
        docu.add_paragraph()
        ...
        docu.add_paragraph()._element.append(latex_to_word(res_r.ansx2))
        docu.add_paragraph()
