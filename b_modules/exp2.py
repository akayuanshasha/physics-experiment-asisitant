...
def schema():
    _s1 = load_sample_data("exp2", "表面张力（测量弹簧劲度系数）")
    sample1 = []
    for row in _s1:
        if len(row) >= 2:
            try:
                sample1.append([float(row[0]), float(row[1])])
    ...
    return make_schema(
        ...
        [
            make_table(
                ...
                ["砝码质量 m(g)", "升降杆读数 x(cm)",
                 "伸长量 Δx(cm)", "拉力 F(mN)"],
                sample=sample1, readonly=(2, 3), initial_rows=10, chart=chart1,
            ),
            make_table(
                ...
                sample=(), readonly=(7, 8, 9), initial_rows=2,
            ),
            make_table(
                "table3", "不同浓度洗洁精表面张力计算表",
                ["浓度 C(%)", "初始读数 l₀(cm)",
                 "破裂均值 l̄(cm)", "表面张力 σ(mN/m)"],
                sample=(), readonly=(3,), initial_rows=3,
