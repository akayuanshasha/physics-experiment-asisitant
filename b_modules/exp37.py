...
def schema():
    calibration_rows = load_sample_data_numeric("exp37", "exp37_example")
    standard_sample = [
        {
            "source": "标准光源",
            "line_label": f"标准线 {index}",
            "pixel_x": row[0],
            "wavelength_nm": row[1],
        }
    ...
    unknown_sample = [
        {
            "source": "待测光源",
            "line_label": f"谱线 {index}",
            "pixel_x": row[0],
            "reference_nm": row[1],
        }
        for index, row in enumerate(calibration_rows[-2:], 1)
        if len(row) >= 2
    ]
    scan_sample = []
    ...
    return {
        "schema_version": 2,
        "description": (
            "本实验分为摄谱仪和单色仪两条流程。摄谱仪流程：氦、汞、钠三种光源各上传左/中/右三张谱图，"
            "系统完成增强、拼接和候选谱线峰检测，峰位需结合原图人工复核后填入定标表；"
            "单色仪流程：测量滤波片和溶液的吸收/透射曲线。"
        ),
        "parameters": [
            {
        ...
        "tables": [
            ...
            },
            {
                ...
                "columns": [
                    {"id": "source", "label": "光源"}, {"id": "line_label", "label": "谱线标识"},
                    {"id": "pixel_x", "label": "峰中心像素x", "unit": "px"},
                    {"id": "reference_nm", "label": "参考波长 λ", "unit": "nm"},
                ],
