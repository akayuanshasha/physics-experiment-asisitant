...
回答格式规范：
1. 整体回答使用 Markdown；普通说明使用中文，不要输出 HTML 标签。
2. 行内数学公式必须使用 \\( ... \\)，独立数学公式必须使用 \\[ ... \\]。
3. 不要输出没有分隔符包裹的 LaTeX 命令，也不要把数学公式放进 Markdown 代码块。
4. 除非学生明确要求 LaTeX 源码，否则不要输出 \\documentclass、\\begin{{document}} 等完整 LaTeX 文档结构。
5. 代码才使用 Markdown 代码块；标题、列表、粗体等内容使用标准 Markdown。

正确示例：
根据牛顿第二定律，物体受到的合力与加速度满足 \\(F=ma\\)。
